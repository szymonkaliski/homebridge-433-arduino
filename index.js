...
function ArduinoSwitchPlatform(log, config) {
  ...
  self.config = config;
  self.log = log;
  if (config.input_output_timeout)
    inputOutputTimeout = config.input_output_timeout;
  if (config.serial_port_in === config.serial_port_out) {
    ...
    self.log(
      "Enabling two-arduino-mode using ",
      config.serial_port_in,
      config.serial_port_out
    );
  }
}
...
ArduinoSwitchPlatform.prototype.accessories = function(callback) {
  ...
  self.config.switches.forEach(function(sw) {
    self.accessories.push(
      new ArduinoSwitchAccessory(sw, self.log, self.config)
    );
  });
  self.config.buttons.forEach(function(sw) {
    self.accessories.push(
      new ArduinoButtonAccessory(sw, self.log, self.config)
    );
  });
...
function ArduinoSwitchAccessory(sw, log, config) {
  ...
  self.notifyOn = helpers.throttle(
    function() {
      self.log("Received on code for %s", self.sw.name);
      self.currentState = true;
      self.service
        .getCharacteristic(Characteristic.On)
        .updateValue(self.currentState);
    },
    self.throttle,
    self
  );
  self.notifyOff = helpers.throttle(
    function() {
...
ArduinoSwitchAccessory.prototype.getServices = function() {
  ...
  service
    ...
    .setCharacteristic(
      ...
      self.sw.on.code + "-" + self.sw.off.code
    )
...
function Device(serial) {
  ...
  serial.on("data", function(data) {
    if (data.indexOf("OK") >= 0) device.processQueue();
  });
}
